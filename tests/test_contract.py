...
``docs/capabilities.json`` in this module is hand-authored (see
``test_capabilities_contract.py``) — this gate owns ONLY ``docs/llms.txt``,
the one file these targets are allowed to regenerate. A hand-written
``llms.txt`` would rot exactly like a hand-written ``capabilities.json``
did (0.8.1 shipping with a stale version): nothing but a model reads it, and
a model cannot tell a stale surface slice from a current one.
"""
