...
``CdnErrorKeysView`` existed since the port but no ``urls*.py`` mounted it —
in *any* stapel library. stapel-translate's ``error_collector`` polls
...
from stapel_cdn.errors import CDN_ERRORS, CdnErrorKeysView


def test_route_is_mounted_and_resolves_to_the_view():
    path = reverse("error-keys")
    assert path.endswith("/error-keys/")
    assert resolve(path).func.view_class is CdnErrorKeysView
...
def test_staff_gets_the_registry_as_json():
    ...
    assert set(CDN_ERRORS) <= set(payload)
