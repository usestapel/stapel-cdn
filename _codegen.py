"""stapel-cdn contract-emission harness (contract-pipeline.md §2-3).

Emits the module's own contract triad into ``docs/`` from a single-module
``{cdn + core}`` Django instance mounted at the canonical ``/cdn/api/v1``
...
Usage:
    python -m stapel_cdn._codegen --out docs        # `make contract`
"""
...
def _configure() -> None:
    ...
    # flat-layout conftest does, so `import cdn`-shaped collisions cannot
    # shadow anything.
    ...
    if not settings.configured:
        from stapel_cdn._codegen_settings import settings_kwargs

        settings.configure(
            **settings_kwargs(root_urlconf="stapel_cdn.codegen_urls", contract=True)
        )
    ...
    # but "/cdn/api" in a single-module harness, which would strip it to bare
    # anonymous names. Pin it to the aggregate convention.
    from drf_spectacular.settings import spectacular_settings

    from stapel_cdn._codegen_settings import CODEGEN_SCHEMA_PATH_PREFIX
    ...
    # so without this the protected endpoints would emit without their
    # `security` entry.
...
def _require_python_312() -> None:
    ...
    if sys.version_info[:2] != (3, 12):
        ...
        raise SystemExit(
            f"stapel-cdn contract emission ABORTED: running Python {got}, "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="stapel-cdn-contract",
        description="Emit this module's contract triad (schema.json + flows.json "
        "+ errors.json) into --out, canonical /cdn/api/v1 prefix.",
    )
    ...
    print(
        f"stapel-cdn contract: {paths} paths, {flows} flows, {errors} error keys "
        f"-> {out}/",
        file=sys.stderr,
    )
    return 0
