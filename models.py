...
class Image(models.Model):
    ...
    is_processed = models.BooleanField(
        default=False,
        help_text="Whether variants have been generated"
    )
    processing_log = models.TextField(
        blank=True,
        default='',
        help_text="Log of processing operations"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['file_hash', 'type'], name='cdn_image_hash_type_unique'),
        ]
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.pk and self.original:
            ...
            if not self.original_width or not self.original_height:
                try:
                    import pyvips
                    img = pyvips.Image.new_from_file(self.original.path, access='sequential')
...
def generate_image_variants_on_save(sender, instance, created, **kwargs):
    ...
    if created and instance.original and not instance.is_processed:
        try:
            ...
            logger.warning(f"Celery not available, processing image {instance.id} synchronously: {e}")
...
def generate_video_variants_on_save(sender, instance, created, **kwargs):
    ...
    if created and instance.original and not instance.is_processed:
        ...
        try:
            ...
            import logging
            logger = logging.getLogger(__name__)
            logger.error(f"Failed to generate variants for video {instance.file_hash}: {str(e)}")